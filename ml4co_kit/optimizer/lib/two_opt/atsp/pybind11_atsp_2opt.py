"""
PyBind11-backed 2-opt local search for ATSP.
"""
...
from .pybind11_impl import pybind11_atsp_2opt_impl


def _pybind11_atsp_2opt_ls(
    ...
    """
    Two-opt local search for ATSP problems using the PyBind11 extension.

    Args:
        init_tour: (B, V+1) or (V+1)
        dists: (B, V, V) or (V, V)
...
def pybind11_atsp_2opt_ls(
    ...
    # Perform local search
    optimized_tour = _pybind11_atsp_2opt_ls(
...
def pybind11_atsp_2opt_batch_ls(
    batch_task_data: List[ATSPTask],
    max_iters: int = 5000,
    type_2opt: int = 2
) -> None:
    """Run 2-opt local search and update the task data in-place."""
    ...
    dists = np.array([task_data.dists for task_data in batch_task_data])

    # Perform local search
    optimized_tours = _pybind11_atsp_2opt_ls(
