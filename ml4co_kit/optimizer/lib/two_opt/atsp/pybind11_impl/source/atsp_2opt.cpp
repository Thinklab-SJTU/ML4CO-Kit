...
/**
 * @brief Two-opt local search for ATSP using PyBind11
 * 
 * Optimizes one or multiple ATSP tours using the two-opt local search algorithm.
...
inline py::array_t<int> two_opt_local_search(
    ...
    if (!batched) {
        ...
        atsp::two_opt(tours_ptr, dists_ptr, num_nodes, num_steps, type_2opt);
    } else {
...
/**
 * @brief PyBind11 module for 2-opt local search for ATSP
 * 
 * Exposes the two_opt_local_search function to Python.
 */
PYBIND11_MODULE(atsp_2opt_impl, m) {
    m.doc() = "PyBind11 implementation of 2-opt for ATSP.";
    ...
    m.def(
        ...
        py::arg("type_2opt"),
        py::arg("num_workers"),
        "Run 2-opt local search for ATSP given a distance matrix.\n"
        "type_2opt=2: Find the best pair in each iteration.\n"
        "type_2opt=1: Return as soon as an improving pair is found.\n"
        "Note: For ATSP, nodes are inserted without path reversal."
    );
}
