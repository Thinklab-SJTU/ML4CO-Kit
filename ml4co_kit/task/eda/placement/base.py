r"""
Base class for Placement tasks.
"""
...
class PlacementTask(EDATaskBase):
    ...
    def evaluate(self, sol: np.ndarray = None, ref: bool = False):
        ...
        target_sol = self.ref_sol if ref else self.sol
        if target_sol is None:
            raise ValueError("Solution is None")
        
        hpwl = self._compute_hpwl(target_sol)
        overlap = self._compute_overlap(target_sol)
        oob = self._compute_oob(target_sol)
        
        obj = self.w_hpwl * hpwl + self.w_overlap * overlap + self.w_oob * oob
