...
- Bins: 一维数组 B = {B_j}，表示这一实例中给定的一批箱子（每个箱子最多使用一次）
...
class BPPTask(TaskBase):
    ...
    bin_sizes : np.ndarray
        一维数组，长度为 n_bins，表示这一实例中给定的所有箱子容量。
    sol :
    ...
    说明：bin_type_indices[k] 即表示“第 k 个使用的 bin 对应 bin_sizes 中的哪个箱子索引 j”，
        同一个 j 在一个解中原则上只应该出现一次（每个箱子只用一次），
        这一约束由求解器保证。
    """
