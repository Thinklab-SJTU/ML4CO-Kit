...
class USATCGenerator(SATGeneratorBase):
    ...
    def _create_instance(self, gen_func: Callable) -> USATCTask:
        # Generate UNSAT clauses (until the clauses are unsatisfiable)
        while True:
            clauses = gen_func()
        ...
        # Create Unsat-Core task
        task_data = USATCTask(precision=self.precision)
        task_data.from_data(clauses=clauses)
        return task_data

    def _generate_phase(self) -> USATCTask:
        return self._create_instance(self._super_generate_phase)

    def _generate_sr(self) -> USATCTask:
        # Call `_super_generate_sr` to generate UNSAT clauses
        unsat_clauses, _, _ = self._super_generate_sr()
        
        # Create Unsat-Core task
        task_data = USATCTask(precision=self.precision)
        task_data.from_data(clauses=unsat_clauses)
        return task_data

    def _generate_ca(self) -> USATCTask:
        return self._create_instance(self._super_generate_ca)

    def _generate_ps(self) -> USATCTask:
    ...
    def _generate_k_domset(self) -> USATCTask:
        return self._create_instance(self._super_generate_k_domset)

    def _generate_k_vercov(self) -> USATCTask:
