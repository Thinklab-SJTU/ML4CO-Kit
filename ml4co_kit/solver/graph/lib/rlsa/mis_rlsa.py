...
def mis_rlsa(
    ...
    energy, grad = mis_energy_func(A, x, b, rlsa_beta, True)
    ...
    while mask.any():
        ...
        solutions[indices, next_node] = 1
        mask[indices,next_node] = False
        nnx_masks = edge_index[0].unsqueeze(0) == next_node.unsqueeze(1)  # Create masks for matching nodes
...
def mis_energy_func(
    ...
    L = A@x
    energy = torch.sum(x*(beta*L+b),dim=0)
    grad = beta*L+b if compute_grad else None
    return energy, grad