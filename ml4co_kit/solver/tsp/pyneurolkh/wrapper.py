...
from ml4co_kit.solver.tsp.pyneurolkh.alkh import neurolkh_alkh_tool_v1, neurolkh_alkh_tool_v2
...
def neurolkh_wrapper(
    ...
    f_edge_index = full_edge_index[1].reshape(nodes_num, sparse_factor)
    ...
    # initial_period
    initial_period = np.power(2, int(np.log10(nodes_num)) + 2) - 1
    ...
    # files to generate
    name = uuid.uuid4().hex
    tmp_tsp_path = f"{name}.tsp" 
    tmp_pi_path = f"{name}.pi" 
    tmp_par_path = f"{name}.par"
    tmp_tour_path = f"{name}.tour"
    tmp_cand_path = f"{name}.cand"
    tmp_log = f"{name}.log"
    ...
    tmp_file_list = [
        ...
        tmp_log, tmp_tour_path, tmp_cand_path
    ]
...
def alkh_wrapper(
    ...
    # delete tmp files
    tmp_file_list = [
        tmp_tsp_path, tmp_pi_path, tmp_par_path, tmp_log, tmp_tour_path
    ]
    for file_path in tmp_file_list:
        if os.path.exists(file_path):
            os.remove(file_path)
