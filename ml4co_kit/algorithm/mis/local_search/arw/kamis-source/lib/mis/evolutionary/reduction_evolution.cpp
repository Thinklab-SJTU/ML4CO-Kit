...
template <typename reducer>
void reduction_evolution<reducer>::perform_evo_mis(MISConfig & mis_config, graph_access & G) {
    srand(mis_config.seed);
    random_functions::setSeed(mis_config.seed);
    mis_log::instance()->restart_evo_timer();
    ...
    individuum_mis best;
    collect_best_mis(mis_config, G, best);
    forall_nodes(G, node) {
        G.setPartitionIndex(node, best.solution[node]);
    } endfor
}
