import numpy as np
from ml4co_kit.algorithm.mis.c_arw import c_mis_ils


def mis_ils_local_search(
    initial_sol: np.ndarray, xadj: np.ndarray, adjncy: np.ndarray, 
    max_iterations: int = 5000, use_three_ls: bool = False
) -> np.ndarray:
    ...
    c_mis_ils(nodes_num, xadj, adjncy, initial_sol, max_iterations, int(use_three_ls), output)
