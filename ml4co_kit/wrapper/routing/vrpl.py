...
class VRPLWrapper(CVRPWrapper):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(VRPLWrapper, self).__init__(precision=precision)
        self.task_type = TASK_TYPE.VRPL
        self.task_list: List[VRPLTask] = list()
        self.task_class = VRPLTask
    ...
    def from_txt(
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                # Parse capacity and max route length
                ...
                if overwrite:
                    self.task_list.append(vrpl_task)
    ...
    def to_txt(
        ...
        with open(file_path, mode) as f:
            ...
            for task in tqdm_by_time(self.task_list, write_msg, show_time):
                ...
                f.write(" max_route_length " + str(max_route_length))
                f.write(str(" output "))
