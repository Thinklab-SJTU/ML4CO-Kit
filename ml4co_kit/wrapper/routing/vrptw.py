r"""
VRPTW Wrapper.
"""

# Copyright (c) 2024 Thinklab@SJTU
# ML4CO-Kit is licensed under Mulan PSL v2.
# You can use this software according to the terms and conditions of the Mulan PSL v2.
# You may obtain a copy of Mulan PSL v2 at:
# http://license.coscl.org.cn/MulanPSL2
#
# THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
# EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
# MERCHANTABILITY OR FIT FOR A PARTICULAR PURPOSE.
# See the Mulan PSL v2 for more details.


import pathlib
import numpy as np
from typing import Union, List
from ml4co_kit.task.base import TASK_TYPE
from ml4co_kit.task.routing.vrptw import VRPTWTask
from ml4co_kit.wrapper.routing.cvrp import CVRPWrapper
from ml4co_kit.utils.time_utils import tqdm_by_time
from ml4co_kit.utils.file_utils import check_file_path
from ml4co_kit.task.routing.base import DISTANCE_TYPE, ROUND_TYPE


class VRPTWWrapper(CVRPWrapper):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(VRPTWWrapper, self).__init__(precision=precision)
        self.task_type = TASK_TYPE.VRPTW
        self.task_list: List[VRPTWTask] = list()
        self.task_class = VRPTWTask

    def from_txt(
        self,
        file_path: pathlib.Path,
        distance_type: DISTANCE_TYPE = DISTANCE_TYPE.EUC_2D,
        round_type: ROUND_TYPE = ROUND_TYPE.NO,
        ref: bool = False,
        overwrite: bool = True,
        normalize: bool = False,
        show_time: bool = False
    ):
        """Read task data from ``.txt`` file"""
        # Overwrite task list if overwrite is True
        if overwrite:
            self.task_list: List[VRPTWTask] = list()

        with open(file_path, "r") as file:
            load_msg = f"Loading data from {file_path}"
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                # Load data
                line = line.strip()
                split_line_0 = line.split("depots ")[1]
                split_line_1 = split_line_0.split(" points ")
                depot = split_line_1[0]
                split_line_2 = split_line_1[1].split(" demands ")
                points = split_line_2[0]
                split_line_3 = split_line_2[1].split(" capacity ")
                demands = split_line_3[0]
                split_line_4 = split_line_3[1].split(" time_windows ")
                capacity = split_line_4[0]
                split_line_5 = split_line_4[1].split(" service_times ")
                time_windows = split_line_5[0]
                split_line_6 = split_line_5[1].split(" output ")
                service_times = split_line_6[0]
                tour = split_line_6[1]

                # Parse depot coordinates
                depot = depot.split(" ")
                depot = np.array([float(depot[0]), float(depot[1])], dtype=self.precision)

                # Parse points coordinates
                points = points.split(" ")
                points = np.array(
                    [
                        [float(points[i]), float(points[i + 1])]
                        for i in range(0, len(points), 2)
                    ], dtype=self.precision
                )

                # Parse demands
                demands = demands.split(" ")
                demands = np.array(
                    [float(demands[i]) for i in range(len(demands))], dtype=self.precision
                )

                # Parse capacity
                capacity = float(capacity)

                # Parse time windows and service times
                time_windows = time_windows.split(" ")
                time_windows = np.array(
                    [float(time_windows[i]) for i in range(len(time_windows))],
                    dtype=self.precision
                ).reshape(-1, 2)
                service_times = service_times.split(" ")
                service_times = np.array(
                    [float(service_times[i]) for i in range(len(service_times))],
                    dtype=self.precision
                )

                # Parse tour
                tour = tour.split(" ")
                tour = np.array([int(t) for t in tour])

                # Create a new task and add it to ``self.task_list``
                if overwrite:
                    vrptw_task = VRPTWTask(
                        distance_type=distance_type,
                        round_type=round_type,
                        precision=self.precision
                    )
                else:
                    vrptw_task = self.task_list[idx]
                vrptw_task.from_data(
                    depots=depot,
                    points=points,
                    demands=demands,
                    capacity=capacity,
                    time_windows=time_windows,
                    service_times=service_times,
                    sol=tour,
                    ref=ref,
                    normalize=normalize
                )
                if overwrite:
                    self.task_list.append(vrptw_task)

    def to_txt(
        self, file_path: pathlib.Path, show_time: bool = False, mode: str = "w"
    ):
        """Write task data to ``.txt`` file"""
        # Check file path
        check_file_path(file_path)

        # Save task data to ``.txt`` file
        with open(file_path, mode) as f:
            write_msg = f"Writing data to {file_path}"
            for task in tqdm_by_time(self.task_list, write_msg, show_time):
                # Check data and get variables
                task._check_depots_not_none()
                task._check_points_not_none()
                task._check_demands_not_none()
                task._check_capacity_not_none()
                task._check_time_windows_not_none()
                task._check_service_times_not_none()
                task._check_sol_not_none()

                depot = task.depots
                points = task.points
                demands = task.demands
                capacity = task.capacity
                time_windows = task.time_windows
                service_times = task.service_times
                sol = task.sol

                # Write data to ``.txt`` file
                f.write("depots " + str(depot[0]) + str(" ") + str(depot[1]))
                f.write(" points" + str(" "))
                f.write(
                    " ".join(
                        str(x) + str(" ") + str(y)
                        for x, y in points
                    )
                )
                f.write(" demands " + str(" ").join(str(demand) for demand in demands))
                f.write(" capacity " + str(capacity))
                f.write(" time_windows ")
                f.write(str(" ").join(str(value) for value in time_windows.reshape(-1)))
                f.write(" service_times ")
                f.write(str(" ").join(str(value) for value in service_times))
                f.write(str(" output "))
                f.write(str(" ").join(str(node_idx) for node_idx in sol))
                f.write("\n")
            f.close()
