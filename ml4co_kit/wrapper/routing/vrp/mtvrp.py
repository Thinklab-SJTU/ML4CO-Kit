...
class MTVRPWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(MTVRPWrapper, self).__init__(
            task_type=TASK_TYPE.MTVRP, precision=precision
        )
        self.task_list: List[MTVRPTask] = list()
        self.task_class = MTVRPTask
    ...
    def from_txt(
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                split_line_4 = split_line_3[1].split(" tw ")
                capacity = split_line_4[0]
                split_line_5 = split_line_4[1].split(" service ")
                tw = split_line_5[0]
                ...
                if tw != "None":
                    ...
                    tw = np.array(
                        [
                            [float(tw[i]), float(tw[i + 1])]
                            for i in range(0, len(tw), 2)
                        ], dtype=self.precision
                    )
                    tw_flag = True
                else:
                ...
                if service != "None":
                    ...
                    service = np.array([
                        float(service[i]) for i in range(len(service))], 
                        dtype=self.precision
                    )
                else:
                    service = None
                ...
                if overwrite:
                    mtvrp_task = MTVRPTask(
                        cvrp_open=cvrp_open,
                        backhaul_flag=backhaul_flag,
                        mixed_backhaul=mixed_backhaul,
                        tw_flag=tw_flag,
                        max_route_length_flag=max_route_length_flag,
                        distance_type=distance_type,
                        round_type=round_type,
                        precision=self.precision
                    )
                else:
                    mtvrp_task = self.task_list[idx]
                mtvrp_task.from_data(
                    depots=depot, points=points, demands=demands, 
                    capacity=capacity, tw=tw, service=service, 
                    max_route_length=max_route_length,
                    sol=tour, ref=ref, normalize=normalize
                )
                if overwrite:
                    self.task_list.append(mtvrp_task)
    ...
    def to_txt(
        ...
        with open(file_path, mode) as f:
            ...
            for task in tqdm_by_time(self.task_list, write_msg, show_time):
                ...
                tw_flag = task.tw_flag
                max_route_length_flag = task.max_route_length_flag
                if tw_flag:
                    task._check_tw_not_none()
                    task._check_service_not_none()
                if max_route_length_flag:
                ...
                if tw_flag:
                    ...
                    f.write(" service " + str(" ").join(str(s) for s in service))
                else:
                    f.write(str(" tw None service None"))
                if max_route_length_flag:
