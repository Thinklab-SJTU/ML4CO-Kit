...
from ml4co_kit.task.routing.vrp.cvrpl import CVRPLTask
from ml4co_kit.utils.file_utils import check_file_path
from ml4co_kit.task.routing.base import DISTANCE_TYPE, ROUND_TYPE


class CVRPLWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPLWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPL, precision=precision
        )
        self.task_list: List[CVRPLTask] = list()
        self.task_class = CVRPLTask
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                if overwrite:
                    ...
                    cvrpl_task = self.task_list[idx]
                cvrpl_task.from_data(
                ...
                if overwrite:
                    self.task_list.append(cvrpl_task)
