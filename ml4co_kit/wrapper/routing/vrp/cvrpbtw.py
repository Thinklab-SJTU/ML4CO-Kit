...
from ml4co_kit.task.routing.vrp.cvrpbtw import CVRPBTWTask
from ml4co_kit.task.routing.base import DISTANCE_TYPE, ROUND_TYPE


class CVRPBTWWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPBTWWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPBTW, precision=precision
        )
        self.task_list: List[CVRPBTWTask] = list()
        self.task_class = CVRPBTWTask
