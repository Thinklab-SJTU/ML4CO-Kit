...
class CVRPTWWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPTWWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPTW, precision=precision
        )
        self.task_list: List[CVRPTWTask] = list()
        self.task_class = CVRPTWTask
