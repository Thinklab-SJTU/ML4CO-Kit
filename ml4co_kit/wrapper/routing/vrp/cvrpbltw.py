...
class CVRPBLTWWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPBLTWWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPBLTW, precision=precision
        )
        self.task_list: List[CVRPBLTWTask] = list()
        self.task_class = CVRPBLTWTask
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                if overwrite:
                    ...
                    cvrpbltw_task = self.task_list[idx]
                cvrpbltw_task.from_data(
                ...
                if overwrite:
                    self.task_list.append(cvrpbltw_task)
