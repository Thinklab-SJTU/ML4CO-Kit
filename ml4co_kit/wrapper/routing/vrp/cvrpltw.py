...
class CVRPLTWWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPLTWWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPLTW, precision=precision
        )
        self.task_list: List[CVRPLTWTask] = list()
        self.task_class = CVRPLTWTask
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                if overwrite:
                    ...
                    cvrpltw_task = self.task_list[idx]
                cvrpltw_task.from_data(
                ...
                if overwrite:
                    self.task_list.append(cvrpltw_task)
