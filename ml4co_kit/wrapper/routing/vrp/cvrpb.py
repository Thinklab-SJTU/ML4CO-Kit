...
class CVRPBWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(CVRPBWrapper, self).__init__(
            task_type=TASK_TYPE.CVRPB, precision=precision
        )
        self.task_list: List[CVRPBTask] = list()
        self.task_class = CVRPBTask
