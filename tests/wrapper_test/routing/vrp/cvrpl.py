...
class CVRPLWrapperTester(WrapperTesterBase):
    def __init__(self):
        super(CVRPLWrapperTester, self).__init__(
            test_wrapper_class=CVRPLWrapper,
            generator=CVRPLGenerator(),
            solver=PyVRPSolver(),
            pickle_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpl/wrapper/cvrpl50_uniform_16ins.pkl"),
            ],
            txt_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpl/wrapper/cvrpl50_uniform_16ins.txt"),
            ],
        )
