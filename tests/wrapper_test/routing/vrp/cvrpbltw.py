...
class CVRPBLTWWrapperTester(WrapperTesterBase):
    def __init__(self):
        super(CVRPBLTWWrapperTester, self).__init__(
            test_wrapper_class=CVRPBLTWWrapper,
            generator=CVRPBLTWGenerator(),
            solver=PyVRPSolver(),
            pickle_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpbltw/wrapper/cvrpbltw50_uniform_16ins.pkl"),
            ],
            txt_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpbltw/wrapper/cvrpbltw50_uniform_16ins.txt"),
            ],
        )
