...
class OVRPTWWrapperTester(WrapperTesterBase):
    def __init__(self):
        super(OVRPTWWrapperTester, self).__init__(
            test_wrapper_class=CVRPTWWrapper,
            generator=CVRPTWGenerator(cvrp_open=True),
            solver=PyVRPSolver(),
            pickle_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrptw/wrapper/ovrptw50_uniform_16ins.pkl"),
            ],
            txt_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrptw/wrapper/ovrptw50_uniform_16ins.txt"),
            ],
            from_txt_args_list=[
                {"cvrp_open": True},
            ],
