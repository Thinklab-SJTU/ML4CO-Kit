...
class CVRPBWrapperTester(WrapperTesterBase):
    def __init__(self):
        super(CVRPBWrapperTester, self).__init__(
            test_wrapper_class=CVRPBWrapper,
            generator=CVRPBGenerator(),
            solver=PyVRPSolver(),
            pickle_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpb/wrapper/cvrpb50_uniform_16ins.pkl"),
            ],
            txt_files_list=[
                pathlib.Path("test_dataset/routing/vrp/cvrpb/wrapper/cvrpb50_uniform_16ins.txt"),
            ],
        )
