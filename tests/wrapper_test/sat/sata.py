...
import pathlib
from ml4co_kit import SATAWrapper, SATAGenerator, ILSSolver
...
class SATAWrapperTester(WrapperTesterBase):
    def __init__(self):
        super(SATAWrapperTester, self).__init__(
            ...
            solver=ILSSolver(),
            pickle_files_list=[
                pathlib.Path("test_dataset/sata/wrapper/sata50_uniform_4ins.pkl"),
            ],
            txt_files_list=[
                pathlib.Path("test_dataset/sata/wrapper/sata50_uniform_4ins.txt"),
            ],
        )
