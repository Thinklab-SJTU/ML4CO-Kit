...
class TSPTaskTester(TaskTesterBase):
    ...
    def _test_other_rw_methods(self):
        ...
        task = TSPTask()
        task.from_tsplib(
            tsp_file_path=pathlib.Path("test_dataset/routing/tsp/tsp/tsplib/problem/a280.tsp"),
            tour_file_path=pathlib.Path("test_dataset/routing/tsp/tsp/tsplib/solution/a280.opt.tour"),
        )
    ...
    def _test_render(self):
        # Read data
        task = TSPTask()
        task.from_pickle("test_dataset/routing/tsp/tsp/task/tsp50_cluster_task.pkl")
